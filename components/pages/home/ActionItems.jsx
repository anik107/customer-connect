...
const ActionItems = () => {
  ...
  if (loading) {
    content = (
      <tr>
        <td colSpan={11} className="whitespace-nowrap p-2 text-center sm:p-3">
    ...
    content = sortedActionItems.map((row, index) => (
      ...
      >
        <td className="sticky left-0 whitespace-nowrap border-r bg-white p-2 dark:bg-slate-800 sm:p-3">
          <div className="w-[16px] min-w-[16px] truncate font-bold" title={index + 1}>
            {index + 1}
          </div>
        </td>
        ...
        <td className="whitespace-nowrap p-2 capitalize sm:p-3">{row.type}</td>
        <td className="whitespace-nowrap p-2 sm:p-3">{row.author_name}</td>
        ...
        <td className="whitespace-nowrap p-2 sm:p-3">{row.sentiment}</td>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <CardDescription className="mt-1 text-sm">
              Recommended actions based on sentiment analysis for the{" "}
              ...
              <thead className="sticky top-0 z-30 bg-slate-50 dark:bg-slate-700">
                <tr className="border-b">
                  <th className="sticky left-0 top-0 z-[50] whitespace-nowrap border-r bg-slate-50 p-2 text-left font-medium dark:bg-slate-700 sm:p-3">
                    <div className="w-[32px] min-w-[32px]">#</div>
                  </th>
                  <th className="sticky top-0 z-40 min-w-[96px] whitespace-nowrap bg-slate-50 p-2 text-left font-medium dark:bg-slate-700 sm:p-3">
                    {renderSortableHeader("Text", SORTABLE_COLUMNS.text)}
                  </th>
                  <th className="min-w-[72px] whitespace-nowrap p-2 text-left font-medium sm:p-3">
                    {renderSortableHeader("Type", SORTABLE_COLUMNS.type)}
                  </th>
                  <th className="min-w-[112px] whitespace-nowrap p-2 text-left font-medium sm:p-3">
                    {renderSortableHeader("Person Name", SORTABLE_COLUMNS.author_name)}
                  </th>
                  <th className="min-w-[64px] whitespace-nowrap p-2 text-center font-medium sm:p-3">
                    {renderSortableHeader("Share Count", SORTABLE_COLUMNS.share_count, "center")}
                  </th>
                  <th className="min-w-[72px] whitespace-nowrap p-2 text-center font-medium sm:p-3">
                    {renderSortableHeader("Reaction Count", SORTABLE_COLUMNS.reaction_count, "center")}
                  </th>
                  <th className="min-w-[72px] whitespace-nowrap p-2 text-center font-medium sm:p-3">
                    {renderSortableHeader("Comment Count", SORTABLE_COLUMNS.comments_count, "center")}
                  </th>
                  <th className="min-w-[84px] whitespace-nowrap p-2 text-left font-medium sm:p-3">
                    {renderSortableHeader("Sentiment", SORTABLE_COLUMNS.sentiment)}
                  </th>
                  <th className="min-w-[88px] whitespace-nowrap p-2 text-left font-medium sm:p-3">
                    {renderSortableHeader("Category", SORTABLE_COLUMNS.category)}
                  </th>
                  <th className="min-w-[88px] whitespace-nowrap p-2 text-left font-medium sm:p-3">
                    {renderSortableHeader("Emotion", SORTABLE_COLUMNS.emotion)}
                  </th>
                  <th className="min-w-[88px] whitespace-nowrap p-2 text-center font-medium sm:p-3">
                    {renderSortableHeader("Virality Score", SORTABLE_COLUMNS.virality_score, "center")}
                  </th>
                </tr>
              </thead>
              <tbody>{content}</tbody>
