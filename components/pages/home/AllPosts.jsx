...
const getPostsSnapshot = (selectedMonthFilter) => {
	...
	const uniquePosts = [...actionItems, ...topPosts].filter(
		(post, index, posts) => {
			const identity =
				post?.post_id ||
				post?.post_url ||
				`${post?.author_name}-${post?.text}`;
...
const AllPosts = () => {
	...
	const columns = (pageIndex, pageSize) => [
		...
		},
		{
			...
			header: 'Date',
			cell: ({ row }) =>
				row.getValue('post_date')
					? dayjs(row.getValue('post_date')).format('DD MMM, YYYY')
					: '-',
		},
		{
			accessorKey: 'source',
			header: () => (
				<div className="w-[72px] min-w-[72px] text-center">Source</div>
			),
		...
		},
		{
			...
			header: () => <div className="text-center">Reaction</div>,
			cell: ({ row }) => (
				<div className="text-center">
					{row.getValue('reaction_count')}
				</div>
			),
		},
	];
	...
	const visibleEnd =
		totalPosts === 0
			? 0
			: Math.min(limit * (page - 1) + posts.length, totalPosts);
		...
		<div className="mt-3">
			...
			<div className="mt-6 flex items-center justify-between">
				<div className="text-sm text-gray-600">
					Showing {visibleStart} to {visibleEnd} of {totalPosts}{' '}
					results
				</div>
